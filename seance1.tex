...
\begin{frame}[t]{Comparaison visuelle : Microsoft Word / \LaTeX}

\setlength\fboxsep{0pt}
\setlength\fboxrule{0.5pt}

\vspace*{-.25cm}
\fbox{\includegraphics[width=.95\textwidth, trim = 1.2cm 22cm 1.2cm 2.2cm, clip]{img/cifa_word}}

\vspace*{-.25cm}
\flushright
\fbox{\includegraphics[width=.95\textwidth, trim = 1cm 21cm 1cm 2cm, clip]{img/cifa_latex}}
...
\centering
\begin{tikzpicture}
  \node (ooo) at (0,0) {\fbox{\includegraphics[width=.55\textwidth]{img/diapo_ooo}}};
  \node (beamer) at (5.5,2) {\fbox{\includegraphics[width=.55\textwidth, trim = 0cm 0cm 0cm 0cm, clip]{img/diapo_beamer}}};
\end{tikzpicture}
...
\begin{frame}[fragile]
    ...
    \begin{block}{Comparé au classes \texttt{book} et \texttt{report} }
        \begin{itemize}
     \item le titre est sur la même page que le début du texte,
...
\begin{frame}[fragile]
    ...
    \begin{block}{Le paquet \texttt{graphicx} :  }
        \begin{itemize}
    \item  permet l'utilisation de commandes spécifiques pour la gestion des figures (échelle taille, rotation, etc.),
...
Leurs utilités principales:
\begin{itemize}
\item\lstinline[language=Caml]?%? indique que le restant de la ligne est en commentaire
\item\lstinline?$...$? indique une formule mathématique dans du texte
\item\lstinline?{ ... }? indique un groupe (groupe de caractères/mots)
\item\lstinline?\...? indique le début d'une séquence de contrôle
\end{itemize}
...
Essayez d'utliser ces différentes commandes sur votre document \LaTeX{} 
...
Essayez d'utliser les commandes \lstinline?\titlepage?, \lstinline?\clearpage? et \lstinline?\tableofcontents? dans votre document \LaTeX.
...
\medskip
Styles de caractères possibles :
...
Regardez sur votre document \LaTeX{}   l'effet des différentes tailles et des différents styles de caractères.

...
\begin{frame}[fragile]
    ...
    \begin{block}{Options de classe}
        \begin{itemize}
            \item \textbf{Xpt} : changer la taille des caractères à \textbf{X}
...
On insère une commande au début de chaque découpage :
\begin{itemize}
\item\lstinline?\part{titre}?: partie
\item\lstinline?\chapter{titre}?: chapitre (uniquement pour \texttt{report} et \texttt{book})
...
\frametitle{Exercice}
    ...
    \begin{lstlisting}[title={\texttt{exercice.tex}}]
        ...
        \input{£\meta{monfichier1}£} %texte 1 
        \input{£\meta{monfichier2}£} %texte 2
        \input{£\meta{monfichier3}£} %texte 3
...
\begin{itemize}
\item Permet d'insérer une image  simplement (format : JPG, GIF, PDF...),
...
\lstinline?\includepdf[pages={3,5-8,60}]{nomdufichier}? pour insérer seulement certaines pages d'un PDF
(ici : les pages 3, 5, 6, 7, 8 et 60).
...
\includepdf[pages=-]{£\meta{nomdufichier}£} %fichier PDF entier
...
\begin{frame}[fragile]
\frametitle{Environnement \texttt{equation}}
...
Permet d'insérer une équation mathématique numérotée (on verra plus loin comment y faire référence) :
...
\section{Environnement liste}

%%%%%%%%% SLIDE %%%%%%%%%%%%%%%%%%

\begin{frame}[fragile]
\frametitle{Environnement \texttt{itemize}}
...
\begin{frame}[fragile]
\frametitle{Environnement \texttt{figure}}
...
\begin{figure}[£\meta{position}£]
  \centering
  \includegraphics[£\meta{options}£]{£\meta{nom\_image}£}
\end{figure}
\end{lstlisting}
...
\begin{itemize}
  ...
  \item Les options de \lstinline?\includegraphics? permettent de régler la taille de l'image :
  \begin{itemize}
...
\begin{figure}[£\meta{position}£]
  \centering
  \includegraphics[£\meta{options}£]{£\meta{nom\_image}£}
  \caption{£\meta{legende}£}
  \label{£\meta{etiquette}£}
\end{figure}
\end{lstlisting}
...
\begin{lstlisting}
Voir diapositive \pageref{monlogo}.
\end{lstlisting}
  \end{column}
  \begin{column}{0.45\textwidth}
\myex{Voir diapositive \pageref{monlogo}.}
  \end{column}
\end{columns}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=5cm]{img/latex}
  \caption{Le logo du langage \LaTeX}
  \label{monlogo}
\end{figure}
\end{lstlisting}
...
\bigskip
(À condition qu'il existe une image \texttt{latex.jpg} dans un dossier \texttt{img} !)
...
\begin{figure}[£\meta{position}£] 
  \centering 
  \includegraphics[width=5cm]{£\meta{chemin-image}£}
  \caption{£\meta{titre}£}
  \label{etiquette1}
\end{figure} 
...
\item possibilité d'insérer des équation dans du texte:\\
  ...
  {\myexsize{.9}{\rmfamily On montre facilement que $\rm 1+1=4$ si on est doué.}}
...
\begin{lstlisting}
Si $x < 0$, alors $\sqrt{x}$ n'existe pas.
\end{lstlisting}
%Si $\pi^2 \geq n$, alors $\sqrt{1+1}=0$ ce qui termine la démonstration.
...
\medskip
% Note : \rmfamily, \mathnormal et \marthrm ne servent qu'à modifier la police
% pour obtenir celle d'un document Latex autre que Beamer (article, report…)
{\myex{\rmfamily
Si $\mathnormal{x} < \mathrm{0}$, alors $\sqrt{\mathnormal{x}}$ n'existe pas.
}}
\end{frame}
...
\begin{equation}       % debut de l'environnement equation
  1+1=0                % equation
  \label{eq_zero}      % etiquette de l'equation
...
\medskip
On peut utiliser l'étiquette de l'équation (\lstinline?eq_zero?) pour y faire référence :

\medskip
\begin{columns}
  \begin{column}{0.45\textwidth}
\begin{lstlisting}
Voir £\'e£quation (\ref{eq_zero}).
Voir £\'e£quation \eqref{eq_zero}.
\end{lstlisting}
  \end{column}
  \begin{column}{0.45\textwidth}
\myex{Voir équation (\ref{eq_zero}).

Voir \'equation \eqref{eq_zero}.}
  \end{column}
\end{columns}
...
\begin{itemize}
\item\lstinline?\sqrt{1+2}?: racine carrée \myexinline{$\sqrt{1+2}$}
\item\lstinline?\sqrt[3]{1+2}?: racine carrée n-ième \myexinline{$\sqrt[3]{1+2}$}
\item\lstinline?\frac{1}{2}?: fraction \myexinline{$\frac{1}{2}$}
\item\lstinline?\sin(1+2)?: sinus \myexinline{$\sin(1+2)$}
\item\lstinline?\cos(1+2)?: cosinus \myexinline{$\cos(1+2)$}
\item\lstinline?1^{1+2}? ou \lstinline?1^2?: puissance \myexinline{$1^{1+2}$ ou $1^2$}
\item\lstinline?1_{1+2}? ou \lstinline?1_2?: indice \myexinline{$1_{1+2}$ ou $1_2$}
\item\lstinline?1_{1+2}^{1+3}? ou \lstinline?1_2^3?: puissance ET indice \myexinline{$1_{1+2}^{1+3}$ ou $1_2^3$}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \lstinline? \dot{v} \ddot{v} \dddot{v} \ddddot{v}?: dérivée \myexinline{$\dot{v}$ $\ddot{v}$ $\dddot{v}$ $\ddddot{v}$}
\item \lstinline? \vec{AB}? ou \lstinline?\overrightarrow{AB}?: vecteur \myexinline{$\vec{AB}$} \myexinline{$\overrightarrow{AB}$}
\item \lstinline? \overline{1+i}?: conjugué \myexinline{$\overline{1+i} $}
\item \lstinline? \sum_a^b?: somme \myexinline{$\sum_a^b x$}
\item \lstinline? \int_a^b?: intégrale \myexinline{$\int_a^b y$}
\item \lstinline? \widehat {ABC}?: angle \myexinline{$\widehat {ABC}$}
\item \lstinline? 2x > 0 \mbox{ si } x > 0?: texte \myexinline{$2x > 0 \mbox{ si } x > 0$}
\item \lstinline? \left( \frac{1}{2} \right)?: grandes parenthèses \myexinline{$\left( \frac{1}{2} \right)$}
\end{itemize}

Note : le rendu est différent dans un environnement \texttt{equation} :
\begin{columns}
...
Types de matrices prédéfinis :

\begin{itemize}
\item \lstinline? \begin{pmatrix} 1 & 2 \\ 3 & 4 \end{pmatrix}?: parenthèses \myexinline{$ \begin{pmatrix} 1 & 2 \\ 3 & 4 \end{pmatrix}$}
\item \lstinline? \begin{bmatrix} 1 & 2 \\ 3 & 4 \end{bmatrix}?: crochets \myexinline{$ \begin{bmatrix} 1 & 2 \\ 3 & 4 \end{bmatrix}$}
\item \lstinline? \begin{Bmatrix} 1 & 2 \\ 3 & 4 \end{Bmatrix}?: accolades \myexinline{$ \begin{Bmatrix} 1 & 2 \\ 3 & 4 \end{Bmatrix}$}
\item \lstinline? \begin{vmatrix} 1 & 2 \\ 3 & 4 \end{vmatrix}?: barres \myexinline{$ \begin{vmatrix} 1 & 2 \\ 3 & 4 \end{vmatrix}$}
\item \lstinline? \begin{Vmatrix} 1 & 2 \\ 3 & 4 \end{Vmatrix}?: doubles barres \myexinline{$ \begin{Vmatrix} 1 & 2 \\ 3 & 4 \end{Vmatrix}$}
\end{itemize}
...
%%%%%%%%%%
\begin{frame}[fragile]
\frametitle{Caractères grecs}

\begin{itemize}
  \item Idée: selon le nom de la lettre grecque
  \begin{itemize}
...
\quad \lstinline?\Omega?: donne \myextrueinline{$\Omega$}
...
\frac{1+3+9+o+\frac{1}{2}} {n-m-\lambda}
\right)
\label{titre_eq} %label de l'équation (ex voir équation W)
...
\begin{itemize}
  ...
  \item On l'insère généralement dans un environnement \lstinline?table?.
\end{itemize}
...
%%%%%%%%%%
\begin{frame}[fragile]
\frametitle{Environnement \texttt{tabular}}
...
\begin{table} %debut de l'environnement table
  ...
  \label{titre_tab} %label du tableau (permet de faire une reference)
  \caption{L£\'e£gende du tableau} %legende du tableau
...
\hline
\end{tabular}
\label{titre_tab3} %label de du tableau (ex voir tableau W)
\caption{L\'egende du tableau} %legende du tableau
\end{table}}
...
\item La déclaration des colonnes s'effectue au début dans la deuxième paire d'accolades \lstinline?\begin{tabular}{|l|c|r|}?, avec :
...
\item \lstinline?|? pour un trait vertical entre deux colonnes (Alt Gr + 6 )
\item \lstinline?||? pour un double trait vertical entre deux colonnes
\end{itemize}
\end{itemize}
...
Modifier le tableau précédent en alignant les colonnes à gauche, droite aux centre (l,r,c),
en modifiant le notre de traits verticaux (|,||)
et en modifiant le nombre de traits horizontaux (\lstinline?\hline?).
...
\begin{itemize}
  \item Fusion de \texttt{nb} colonnes : \lstinline?\multicolumn{nb}{c|c|}{texte}?
  \item Fusion de lignes : assez difficile ; pour simplifier on peut tracer
    manuellement les lignes horizontales à la place de \lstinline?\hline?
    entre les colonnes \texttt{nb1} et \texttt{nb2} seulement : \lstinline?\cline{nb1-nb2}?
\end{itemize}
...
La \emph{base de données} bibliographique est placée dans un fichier extérieur (\texttt{.bib}),
et on insère la bibliographie dans le document par les commandes:
\begin{lstlisting}
\bibliographystyle{plain}
\bibliography{£\meta{nom-biblio}£}
\end{lstlisting}
On y fait référence par la commande \lstinline?\cite{...}?  \myextrueinline{\cite{latexcompanion}}
...
Créer un nouveau fichier \texttt{.bib} nommé \texttt{biblio.bib}.
...
\begin{lstlisting}
...
Blablabla vous r£\'e£f£\'e£rer £\`a£ \cite{greenwade93}, ...
...
\bibliographystyle{plain}
\bibliography{biblio}
\end{lstlisting}
...
Sinon, il est possible d'utiliser les styles \emph{abbrv-fr} ou \emph{alpha-fr}.
\end{frame}
...
Par exemple avec Google Scholar: Préférences Scholar / Gestionnaire des bibliographies / Afficher les liens permettant d'importer des citations dans BibTeX).

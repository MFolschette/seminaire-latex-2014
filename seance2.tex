...
Exemple : \myex{Voir diapositive \pageref{etiquette11}.}
...
\end{lstlisting}
  ...
  \begin{column}{0.45\textwidth}
\rmfamily
\myex{Voir équation (\ref{titre_eq}).}
  \end{column}
\end{columns}
...
\item\lstinline?\sin(1+2)?: cosinus \myexinline{$\sin(1+2)$}
\item\lstinline?\cos(1+2)?: sinus \myexinline{$\cos(1+2)$}
\item\lstinline?1^{1+2} ou 1^2?: puissance \myexinline{$1^{1+2}$ ou $1^2$}
\item\lstinline?1_{1+2} ou 1_2?: indice \myexinline{$1_{1+2}$ ou $1_2$}
\item\lstinline?1_{1+2}^{1+3} ou 1_2^3?: puissance ET indice \myexinline{$1_{1+2}^{1+3}$ ou $1_2^3$}
\end{itemize}
\end{frame}
...
\item \lstinline? \left( \frac{1}{2} \right) ?: grande parenthèse \myexinline{$\left( \frac{1}{2} \right)$}
\end{itemize}
